...
class HistoryHfqData : BaseModel() {
    ...
    fun FillBy(stockCode: String, csvRecord: CSVRecord): HistoryHfqData {
        code = stockCode
    ...
    companion object : Model.Find<Long, HistoryHfqData>() {
        ...
        fun CsvToDb(csvFile: File) {
            val code = HistoryNofqData.CodeByFile(csvFile)
            val records  = CSVParser.parse(csvFile, Charset.forName("UTF-8"), CSVFormat.DEFAULT.withFirstRecordAsHeader())
                    .asIterable()
                    .map {
                        val hisData = HistoryHfqData.FindOrCreate(code, it.getJDateTime("date", "YYYY-MM-DD")!!)
        ...
        fun FindOrCreate(code: String, date: JDateTime): HistoryHfqData {
            return HistoryHfqData.where()
                    .eq("code", code)
                    .eq("t_date", date)
                    .findUnique() ?: HistoryHfqData()
        }
    }
}