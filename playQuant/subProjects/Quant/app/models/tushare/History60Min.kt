...
@Entity
@Table(name = "history60min")
class History60Min : BaseModel() {
    ...
    fun FillBy(stockCode: String, csvRecord: CSVRecord): History60Min {
        code = stockCode
    ...
    companion object : Model.Find<Long, History60Min>() {
        ...
        fun FindOrCreate(code: String, date: JDateTime): History60Min {
            return History60Min.where()
                    .eq("code", code)
                    .eq("t_date", date)
                    .findUnique() ?: History60Min()
        }
    }
}