...
@Entity
@Table(name = "history30min")
class History30Min : BaseModel() {
    ...
    fun FillBy(stockCode: String, csvRecord: CSVRecord): History30Min {
        code = stockCode
    ...
    companion object : Model.Find<Long, History30Min>() {
        ...
        fun FindOrCreate(code: String, date: JDateTime): History30Min {
            return History30Min.where()
                    .eq("code", code)
                    .eq("t_date", date)
                    .findUnique() ?: History30Min()
        }
    }
}